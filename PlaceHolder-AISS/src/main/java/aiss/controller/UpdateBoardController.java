...
public class UpdateBoardController extends HttpServlet {
	...
	private static final Logger log = Logger.getLogger(UpdateBoardController.class.getName());
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.log(Level.INFO, "Procesando UpdateBoardController.");
		...
		// Actualizamos el tablero y en caso de error capturamos la excepción
		try {
			log.log(Level.INFO, "Actualizando tablero.");
			boolean updated = autentificacionTrello.updateBoard(id, name);
			...
			if(updated == true) {
				...
				log.log(Level.FINE, "Tablero actualizado correctamente.");
			}
			else {
				request.setAttribute("noUpdated", trelloResults);
				log.log(Level.SEVERE, "No se ha podido actualizar el tablero correctamente.");
			}
			rd = request.getRequestDispatcher("/listaTareas.jsp");
		} catch (Exception e) {
			log.log(Level.WARNING, "Error al actualizar el tablero.");
		}
